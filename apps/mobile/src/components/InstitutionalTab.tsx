...
function StatCard({
  ...
  return (
    <View className="flex-1 items-center rounded-lg border border-zinc-100 bg-zinc-50 p-3">
      <Text className="text-[10px] text-zinc-500">{label}</Text>
      <Text className={`mt-1 text-lg font-bold ${valueColor ?? "text-zinc-900"}`}>
        {value}
      </Text>
      {sub && <Text className="mt-0.5 text-[10px] text-zinc-400">{sub}</Text>}
    </View>
  );
}
  ...
  return (
    <View className="rounded-xl border border-zinc-200 bg-white p-4">
      <View className="mb-3 flex-row items-center justify-between">
        <Text className="text-sm font-semibold text-zinc-700">Options Flow</Text>
          ...
          {(["all", "calls", "puts"] as const).map((f) => (
            <TouchableOpacity
              ...
              className={`rounded-lg px-2.5 py-1 ${
                filter === f ? "bg-primary-600" : "bg-zinc-100"
              }`}
            >
              <Text
      ...
      {filtered.length === 0 ? (
        <Text className="py-6 text-center text-sm text-zinc-400">No options flow data</Text>
      ) : (
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          <View>
            {/* Header */}
            <View className="flex-row border-b border-zinc-100 pb-2">
              <Text className="w-16 text-xs font-medium text-zinc-500">Type</Text>
              <Text className="w-16 text-xs font-medium text-zinc-500">Strike</Text>
              <Text className="w-20 text-xs font-medium text-zinc-500">Expiry</Text>
              <Text className="w-20 text-right text-xs font-medium text-zinc-500">Premium</Text>
              <Text className="w-14 text-right text-xs font-medium text-zinc-500">Qty</Text>
              <Text className="w-20 text-center text-xs font-medium text-zinc-500">Signal</Text>
            </View>
            {/* Rows */}
            {filtered.slice(0, 15).map((t, i) => {
              ...
              return (
                <View
                  key={i}
                  className={`flex-row items-center border-b border-zinc-50 py-2 ${isLarge ? "bg-yellow-50" : ""}`}
                >
                  <Text className="w-16 text-xs font-medium text-zinc-900">{t.type ?? "—"}</Text>
                  <Text className="w-16 text-xs text-zinc-700">
                    {t.strike != null ? `$${t.strike}` : "—"}
                  </Text>
                  <Text className="w-20 text-xs text-zinc-700">{t.expiry ?? "—"}</Text>
                  <Text className={`w-20 text-right text-xs font-medium ${isLarge ? "text-yellow-700" : "text-zinc-900"}`}>
                    {formatCurrency(t.premium)}
                  </Text>
                  <Text className="w-14 text-right text-xs text-zinc-700">
...
function CongressionalSection({ trades }: { trades: CongressionalTrade[] | null }) {
  if (!trades || trades.length === 0) {
    return (
      <View className="rounded-xl border border-zinc-200 bg-white p-4">
        <Text className="mb-2 text-sm font-semibold text-zinc-700">Congressional Trading</Text>
        <Text className="py-4 text-center text-sm text-zinc-400">No congressional trades found</Text>
      </View>
    );
  }
  ...
  return (
    <View className="rounded-xl border border-zinc-200 bg-white p-4">
      <Text className="mb-3 text-sm font-semibold text-zinc-700">Congressional Trading</Text>
      <View className="gap-2">
        {trades.slice(0, 8).map((t, i) => (
          <View key={i} className="rounded-lg border border-zinc-50 bg-zinc-50 p-3">
            ...
            <View className="mt-1 flex-row items-center gap-2">
              ...
              <Text className="text-xs text-zinc-500">{t.amount ?? "—"}</Text>
              <Text className="text-xs text-zinc-400">{formatDate(t.date)}</Text>
            </View>
          </View>
        ))}
      </View>
...
function InsiderSection({ trades }: { trades: InsiderTrade[] | null }) {
  if (!trades || trades.length === 0) {
    return (
      <View className="rounded-xl border border-zinc-200 bg-white p-4">
        <Text className="mb-2 text-sm font-semibold text-zinc-700">Insider Transactions</Text>
        <Text className="py-4 text-center text-sm text-zinc-400">No insider trades found</Text>
      </View>
    );
  }
  ...
  return (
    <View className="rounded-xl border border-zinc-200 bg-white p-4">
      <Text className="mb-3 text-sm font-semibold text-zinc-700">Insider Transactions</Text>
      <View className="gap-2">
        {trades.slice(0, 8).map((t, i) => (
          <View key={i} className="rounded-lg border border-zinc-50 bg-zinc-50 p-3">
            <View className="flex-row items-center justify-between">
              <View className="flex-1">
                ...
                <Text className="text-xs text-zinc-500">{t.title ?? "—"}</Text>
              </View>
              <SentimentBadge sentiment={t.transaction_type} />
            </View>
            <View className="mt-1 flex-row gap-4">
              ...
              <Text className="text-xs text-zinc-500">{formatCurrency(t.value)}</Text>
              <Text className="text-xs text-zinc-400">{formatDate(t.date)}</Text>
            </View>
          </View>
        ))}
...
function HoldingsSection({ holdings }: { holdings: InstitutionalHolding[] | null }) {
  if (!holdings || holdings.length === 0) {
    return (
      <View className="rounded-xl border border-zinc-200 bg-white p-4">
        <Text className="mb-2 text-sm font-semibold text-zinc-700">Institutional Holdings</Text>
        <Text className="py-4 text-center text-sm text-zinc-400">No holdings data available</Text>
      </View>
    );
  }
  ...
  return (
    <View className="rounded-xl border border-zinc-200 bg-white p-4">
      <Text className="mb-3 text-sm font-semibold text-zinc-700">Top Institutional Holders</Text>
      <View className="gap-2">
        {holdings.slice(0, 10).map((h, i) => (
          <View key={i} className="flex-row items-center justify-between border-b border-zinc-50 py-2">
            <View className="flex-1">
...
export default function InstitutionalTab({ ticker }: { ticker: string }) {
  ...
  if (error) {
    return (
      <View className="items-center py-12">
        <Text className="text-zinc-500">Institutional data unavailable</Text>
      </View>
    );
  }
