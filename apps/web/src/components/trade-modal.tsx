    ...
    >
      <div className="w-full max-w-md rounded-2xl border border-gray-200 bg-white shadow-xl">
        ...
        <div className="flex items-center justify-between border-b border-gray-100 px-6 py-4">
          <h2 className="text-lg font-bold text-gray-900">Trade {ticker}</h2>
          <button
            type="button"
            onClick={onClose}
        ...
        {result ? (
          ...
          <div className="px-6 py-8 text-center">
            ...
            <h3 className="mt-4 text-lg font-semibold text-gray-900">Trade Executed</h3>
            <div className="mt-4 rounded-lg bg-gray-50 p-4 text-sm">
              ...
              <div className="flex justify-between py-1">
                ...
                <span className="font-medium text-gray-900">{result.transaction.ticker}</span>
              </div>
              <div className="flex justify-between py-1">
                <span className="text-gray-500">Quantity</span>
                <span className="font-medium text-gray-900">{result.transaction.quantity}</span>
              </div>
              <div className="flex justify-between py-1">
                <span className="text-gray-500">Price</span>
                <span className="font-medium text-gray-900">${parseFloat(result.transaction.price_per_share).toFixed(2)}</span>
              </div>
              <div className="flex justify-between border-t border-gray-200 pt-2 mt-2">
                <span className="font-medium text-gray-700">Total</span>
                <span className="font-bold text-gray-900">${parseFloat(result.transaction.total_amount).toFixed(2)}</span>
              </div>
            </div>
          ...
          <div className="px-6 py-5">
            {portfolioLoading ? (
              ...
              <>
                ...
                {portfolios.length > 1 && (
                  <div className="mt-4">
                    <label htmlFor="trade-portfolio" className="block text-sm font-medium text-gray-600">
                      Portfolio
                    </label>
                    <select
                      id="trade-portfolio"
                ...
                {/* Context Info */}
                <div className="mt-4 rounded-lg bg-gray-50 px-4 py-2.5 text-sm">
                  {side === "buy" ? (
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-gray-900">{sharesQty}</span>
                    </div>
                  )}
                </div>
                ...
                {/* Quantity Input */}
                <div className="mt-4">
                  <label htmlFor="trade-quantity" className="block text-sm font-medium text-gray-600">
                    Shares
                  </label>
                  <input
                    id="trade-quantity"
                    ...
                    className="mt-1 w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm font-medium text-gray-900 placeholder:text-gray-400 focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/20"
                    autoFocus
                  />
                </div>
